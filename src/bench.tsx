  ...
  */
  ...
  isGL() {
    ...
    if (gl) {
      ...
      const addProfiler = (fn:any, self:any, target:any) => function() {
        const t = self.now();
  ...
  nextFrame(now: any) {
    ...
    if (this.frameId <= 1) {
      ...
      if (duration >= 1e3) {
        ...
        for (let i = 0; i < this.names.length; i++) {
          ...
          const mem =
          Math.round(window.performance && window.performance.memory
              ? window.performance.memory.usedJSHeapSize / (1 << 20)
              : 0)
          this.paramLogger({i, cpu, gpu, mem, fps: Math.round(fps), duration: Math.round(duration), frameCount})
    ...
    if (!this.detected || !this.chartFrame) {
      ...
      while (--hz > 0 && this.detected) {
        ...
        for (let i = 0; i < this.names.length; i++) {
          this.chartLogger({i, chart: this.chart, circularId: this.circularId})
        }
        this.circularId++
        this.chartFrame = this.frameId
        this.chartTime = t
